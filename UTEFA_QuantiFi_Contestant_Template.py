...
def update_portfolio(curMarket: Market, curPortfolio: Portfolio, context: Context):
    ...
    # ======================================================
    # ★★★ 8) 方法二：有效前沿优化（Markowitz只优化选中的股票）
    # ======================================================
    stocks = list(final_w.keys())

    # 只保留 ML 筛出的股票（final_w > 0）
    selected = [s for s in stocks if final_w[s] > 1e-8]

    # 如果没有股票被 ML 选中 → 直接空仓
    if len(selected) == 0:
        final_w = {s: 0 for s in stocks}
    else:
        # μ = ML score（方向 & 强弱）
        mu = np.array([scores[s] for s in selected])

        # 协方差矩阵：用过去 20 天收益
        rets = []
        for s in selected:
            p = context.price_history[s][-21:]
            r = np.diff(p) / p[:-1]
            rets.append(r)
        rets = np.array(rets)  # shape (k,20)

        cov = np.cov(rets)

        # Markowitz（最大 Sharpe，long-only）
        def markowitz(mu, cov, reg=1e-3):
            cov_reg = cov + np.eye(len(mu)) * reg
    ...
    MIN_TRADE_VALUE = total_value * 0.007
